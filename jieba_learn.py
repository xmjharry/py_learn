...
wl_space_split = collections.Counter(wordlist_jieba)
print(list(wl_space_split))
# wl_space_split = " ".join(wordlist_jieba)
resource_dir = os.path.join(os.path.dirname(__file__), "resource")
tmp_dir = os.path.join(os.path.dirname(__file__), "rec_tmp")
alice_coloring = np.array(Image.open(os.path.join(resource_dir, "boy_mask.png")))
my_wordcloud = WordCloud(background_color="white", max_words=2000, mask=alice_coloring,
                         max_font_size=40, random_state=42,
                         font_path=os.path.join(resource_dir, './SimHei.ttf')).fit_words(wl_space_split)
